...
import { batch3Articles, type Batch3Slug } from "../batch3Data";

export function BatchArticlePage({ slug }: { slug: Batch3Slug }) {
  const article = batch3Articles[slug];
          ...
          {article.sections.map((section) => (
            <section key={section.heading} className="space-y-3">
              <h2 className="text-base font-black text-[#1A3668] uppercase tracking-wide pt-3 flex items-center gap-2">
                <BookOpenCheck className="w-5 h-5 text-[#FF5F1F]" /> {section.heading}
              </h2>
              ...
              {section.source && (
                <p className="text-xs text-slate-500">
                  Official source: <a href={section.source.url} className="text-[#1A3668] font-bold hover:underline">{section.source.label}</a>.
                </p>
              )}
            </section>
          ))}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3 shadow-sm">
            ...
            <ul className="space-y-2 text-xs">
              {article.related.map((link) => (
                <li key={link.href}><a href={link.href} className="text-[#FF5F1F] font-bold hover:underline">{link.label}</a></li>
              ))}
            </ul>
          </div>
        </article>
