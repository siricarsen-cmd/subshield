...
import { batch5Articles, type Batch5Slug } from "../batch5Data";
import { TopicHubCallout } from "./TopicHubCallout";

export function Batch5ArticlePage({ slug }: { slug: Batch5Slug }) {
  const article = batch5Articles[slug];
  return (
        ...
        <article className="lg:col-span-2 space-y-6 text-sm text-slate-700 leading-relaxed font-medium">
          ...
          {article.sections.map((s) => (
            <section key={s.heading} className="space-y-3">
              <h2 className="text-base font-black text-[#1A3668] uppercase tracking-wide pt-3 flex items-center gap-2"><BookOpenCheck className="w-5 h-5 text-[#FF5F1F]" /> {s.heading}</h2>
              {s.paragraphs?.map((p) => <p key={p}>{p}</p>)}
              {s.bullets && <ul className="list-disc pl-5 space-y-2">{s.bullets.map((b) => <li key={b}>{b}</li>)}</ul>}
              {s.source && <p className="text-xs text-slate-500">Official source: <a href={s.source.url} className="text-[#1A3668] font-bold hover:underline">{s.source.label}</a>.</p>}
            </section>
          ))}
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3 shadow-sm">
            <h3 className="text-xs font-black text-[#1A3668] uppercase tracking-wide">Related SubPreCheck Guides</h3>
            <ul className="space-y-2 text-xs">{article.related.map((l) => <li key={l.href}><a href={l.href} className="text-[#FF5F1F] font-bold hover:underline">{l.label}</a></li>)}</ul>
          </div>
