...
import { ArrowLeft, ShieldAlert, Copy, AlertTriangle, FileText, CheckCircle } from "lucide-react";

export default function SampleReportPage() {
  const handleCopyEmail = () => {
    // In production, this will copy the textarea content to the clipboard
    alert("Pushback email copied to clipboard!");
  };
      ...
      <div className="bg-white border-b border-slate-200 py-4 px-6 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link href="/dashboard" className="inline-flex items-center text-xs font-bold text-[#FF5F1F] hover:text-orange-600 uppercase tracking-widest transition-colors">
            <ArrowLeft size={14} className="mr-2" /> Return to Intake Hub
          </Link>
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-4">
          <h2 className="text-sm font-black text-[#1A3668] uppercase tracking-widest flex items-center gap-2">
            <FileText className="w-4 h-4 text-[#FF5F1F]" /> Consolidated PM Pushback Email Builder
          </h2>
          
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col h-[calc(100vh-280px)] min-h-[600px]">
            <div className="bg-slate-50 border-b border-slate-200 p-4">
              <p className="text-xs font-medium text-slate-500">
                Use this consolidated briefing to immediately push back on predatory terms before engaging outside counsel.
              </p>
            </div>
            <textarea 
              readOnly
...
We've reviewed the draft subcontract for the upcoming project. Before we can execute and begin mobilization, we need to address a few critical liability shifts embedded in the boilerplate that fall outside standard industry parameters. 

Specifically, we need to strike or amend the following:
...
Let me know when you have 10 minutes to jump on a quick call so we can align these terms and get this executed.
...
[Your Name]`}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200">
              <button 
                ...
                className="w-full flex justify-center items-center gap-2 bg-[#FF5F1F] text-white hover:bg-[#E04F1A] py-3 rounded-lg text-xs font-black uppercase tracking-widest transition-all shadow-sm"
              >
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="space-y-4 overflow-y-auto pr-2" style={{ maxHeight: 'calc(100vh - 280px)' }}>
            ...
            {/* Flag 1 */}
            <div className="bg-white border-l-4 border-red-500 border-y border-r border-y-slate-200 border-r-slate-200 rounded-r-xl p-5 shadow-sm">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded p-3">
                <p className="text-[10px] font-black text-[#FF5F1F] uppercase tracking-widest mb-1">Target Redline Alternative Map:</p>
                <p className="text-xs text-slate-700 font-medium">Delete the broad phrase in Article 3 and replace it with: <em className="text-[#1A3668]">"Subcontractor's indemnification obligations under this Article shall apply only to the extent caused by the negligent acts, errors, or omissions of the subcontractor..."</em></p>
              </div>
            </div>
            ...
            {/* Flag 3 */}
            <div className="bg-white border-l-4 border-orange-500 border-y border-r border-y-slate-200 border-r-slate-200 rounded-r-xl p-5 shadow-sm">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2">
