...
await check("incident vocabulary contains only fixed code and severity strings", async () => {
  ...
  assert.equal(Object.keys(OPERATIONAL_INCIDENT_SEVERITIES).length, 18);
...
await check("TypeScript and database incident allowlists match exactly", async () => {
  const migrationSql = readFileSync(
    new URL(
      "../../supabase/migrations/20260803062000_add_operational_incident_monitoring.sql",
      import.meta.url,
    ),
    "utf8",
  );
  const constraintBlock = migrationSql.match(
    /constraint operational_incidents_event_code_check check \([\s\S]*?\n  \)\n\);/,
  )?.[0] ?? "";
  const migrationCodes = [
    ...constraintBlock.matchAll(/'([a-z0-9_]+)'/g),
  ].map((match) => match[1]).sort();

  assert.deepEqual(
    migrationCodes,
    Object.keys(OPERATIONAL_INCIDENT_SEVERITIES).sort(),
  );
});

await check("reservation RPC failures classify as sanitized credit incidents", async () => {
    ...
    await executePaidReview(
      {
        async rpc(functionName) {
          if (functionName === "reserve_review_credit") {
            return { data: null, error: { message: "private database failure" } };
          }
          return { data: null, error: null };
        },
        from() {
          throw new Error("Completed-result lookup should not run.");
        },
      },
      { userId: "00000000-0000-4000-8000-000000000001", auditId: "77" },
      async () => {
        analysisRan = true;
        return { ok: true };
      },
  ...
  assert.equal(analysisRan, false);
  assert.equal(caught instanceof ReviewCreditError, true);
  assert.equal(caught.status, 500);
  assert.equal(caught.code, "reservation_failed");
  assert.equal(caught.message, "Review credit could not be reserved.");
  assert.equal(caught.message.includes("private database failure"), false);
});
