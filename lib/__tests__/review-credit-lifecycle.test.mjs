...
import { normalizeAuditId } from "../audit-id.ts";
...
class MemoryReviewDatabase {
  ...
  async rpc(functionName, params) {
    ...
    if (functionName === "reserve_review_credit") {
      this.lastReservationAuditId = params.p_audit_id;
...
const fileReview = await executePaidReview(database, { userId: userA, auditId: "00015" }, async () => ({ source: "file" }));
const textReview = await executePaidReview(database, { userId: userA, auditId: 16 }, async () => ({ source: "pasted-text" }));
check("file and pasted-text paths submit identical normalized IDs", fileReview.result.source === "file" && textReview.result.source === "pasted-text" && database.reservations.has("15") && database.reservations.has("16") && database.balances.get(userA) === 0);

const failedAudit = "17";
...
const completedDeletionAudit = "21";
const refundedDeletionAudit = "22";
const activeDeletionAudit = "23";
...
check(
  ...
  reservationDefinition.includes("audit_id bigint primary key")
    && !reservationDefinition.includes("references public.contract_audits")
    && !reservationDefinition.includes("on delete cascade"),
);
check("valid bigint audit ID string is accepted", normalizeAuditId("9223372036854775807") === "9223372036854775807");
check("normal numeric database ID is normalized safely", normalizeAuditId(42) === "42");
check("leading zeroes normalize to canonical decimal", normalizeAuditId("00042") === "42");
check("zero audit ID is rejected", normalizeAuditId("0") === null);
check("negative audit ID is rejected", normalizeAuditId("-1") === null);
check("decimal audit ID is rejected", normalizeAuditId("1.5") === null);
check("UUID audit ID is rejected", normalizeAuditId("00000000-0000-4000-8000-000000000011") === null);
check("arbitrary audit ID text is rejected", normalizeAuditId("review-42") === null);
check("unsafe JavaScript numeric audit ID is rejected", normalizeAuditId(Number.MAX_SAFE_INTEGER + 1) === null);
check("out-of-range PostgreSQL bigint audit ID is rejected", normalizeAuditId("9223372036854775808") === null);

const invalidAuditDatabase = new MemoryReviewDatabase();
let invalidAuditDenied = false;
try {
  await executePaidReview(
    invalidAuditDatabase,
    { userId: userA, auditId: Number.MAX_SAFE_INTEGER + 1 },
    async () => ({ shouldNotRun: true }),
  );
} catch (error) {
  invalidAuditDenied = error instanceof ReviewCreditError
    && error.status === 400
    && error.code === "invalid_audit_id";
}
check(
  "unsafe audit input is rejected before analyzer reservation",
  invalidAuditDenied && invalidAuditDatabase.lastReservationAuditId === null,
);

const normalizedRpcDatabase = new MemoryReviewDatabase();
normalizedRpcDatabase.audits.set("9007199254740993", { userId: userA });
normalizedRpcDatabase.balances.set(userA, 1);
await executePaidReview(
  normalizedRpcDatabase,
  { userId: userA, auditId: "09007199254740993" },
  async () => ({ normalized: true }),
);
check(
  "analyzer reservation uses the normalized bigint audit ID",
  normalizedRpcDatabase.lastReservationAuditId === "9007199254740993",
);

const auditRpcNames = [
  "reserve_review_credit",
  "complete_review_credit",
  "refund_review_credit",
  "begin_review_deletion",
  "cancel_review_deletion",
  "finalize_review_deletion",
];
check(
  "all audit RPC definitions and privilege signatures use bigint",
  auditRpcNames.every((name) => new RegExp(
    `create or replace function public\\.${name}\\([\\s\\S]*?p_audit_id bigint`,
  ).test(migrationSql))
    && auditRpcNames.every((name) => migrationSql.includes(`function public.${name}(uuid, bigint`))
    && !/p_audit_id\s+uuid/.test(migrationSql)
    && !/audit_id\s+uuid\s+primary key/.test(migrationSql),
);
check(
  "user identifiers remain uuid in the ledger and RPCs",
  reservationDefinition.includes("user_id uuid not null")
    && auditRpcNames.every((name) => migrationSql.includes(`function public.${name}(uuid, bigint`)),
);
check(
