...
import {
  canDeleteReviewWithReservation,
  canSubmitReview,
...
for (const auditId of [completedDeletionAudit, refundedDeletionAudit]) {
  const reservation = deletionDatabase.reservations.get(auditId);
  if (canDeleteReviewWithReservation(reservation.status)) deletionDatabase.audits.delete(auditId);
}
check(
...
check(
  ...
  !canDeleteReviewWithReservation(deletionDatabase.reservations.get(activeDeletionAudit).status)
...
check(
  "guarded deletion locks the audit and blocks active reservations atomically",
  migrationSql.includes("function public.delete_review_if_not_reserved")
    && migrationSql.includes("return 'reserved';")
