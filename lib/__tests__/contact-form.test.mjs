...
check(
  ...
  successfulResponse.status === 200 &&
    ...
    deliveries[0].subject === "[SubPreCheck Contact] Pricing and Credits" &&
    diagnostics.length === 0,
);

const deliveryCountBeforeRequestGuards = deliveries.length;
const missingOriginResponse = await successfulHandler(
  new Request("https://www.subshield.net/api/contact", {
    method: "POST",
...
check(
  "route rejects a missing Origin without invoking delivery",
  missingOriginResponse.status === 403 && deliveries.length === deliveryCountBeforeRequestGuards,
);

const mismatchedOriginResponse = await successfulHandler(
  new Request("https://www.subshield.net/api/contact", {
    method: "POST",
    headers: {
...
check(
  "route rejects a mismatched Origin without invoking delivery",
  mismatchedOriginResponse.status === 403 && deliveries.length === deliveryCountBeforeRequestGuards,
);

const nonJsonResponse = await successfulHandler(
  new Request("https://www.subshield.net/api/contact", {
    method: "POST",
    headers: {
      Origin: "https://www.subshield.net",
...
check(
  "route rejects non-JSON content without invoking delivery",
  nonJsonResponse.status === 415 && deliveries.length === deliveryCountBeforeRequestGuards,
);

const measuredOversizedResponse = await successfulHandler(
  new Request("https://www.subshield.net/api/contact", {
    method: "POST",
    headers: {
      Origin: "https://www.subshield.net",
    ...
    body: JSON.stringify(submission({ message: "m".repeat(17_000) })),
  }),
);
check(
  "route measures and rejects a body over 16 KB without Content-Length",
  measuredOversizedResponse.status === 413 &&
    deliveries.length === deliveryCountBeforeRequestGuards,
);
