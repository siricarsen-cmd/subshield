...
import {
  ...
  resolveCanonicalProductionOrigin,
} from "../production-origin.ts";
...
const healthyEnvironment = {
  ...
  NEXT_PUBLIC_BASE_URL: SUBSHIELD_PRODUCTION_ORIGIN,
...
function canonicalResolverRejects(baseUrl) {
  try {
    resolveCanonicalProductionOrigin({
      ...healthyEnvironment,
...
check(
  "complete current SubShield production runtime configuration reports ok",
  getProductionHealthStatus(healthyEnvironment) === "ok",
);

check(
  "approved SubPreCheck production runtime configuration reports ok",
  getProductionHealthStatus({
    ...healthyEnvironment,
    NEXT_PUBLIC_BASE_URL: SUBPRECHECK_PRODUCTION_ORIGIN,
  }) === "ok",
);

check(
  "canonical resolver preserves the current SubShield production origin",
  resolveCanonicalProductionOrigin(healthyEnvironment) === SUBSHIELD_PRODUCTION_ORIGIN,
);

check(
  "canonical resolver accepts the future SubPreCheck production origin",
  resolveCanonicalProductionOrigin({
    ...healthyEnvironment,
    NEXT_PUBLIC_BASE_URL: SUBPRECHECK_PRODUCTION_ORIGIN,
  }) === SUBPRECHECK_PRODUCTION_ORIGIN,
...
check(
  "canonical resolver rejects an unexpected production origin",
  canonicalResolverRejects("https://subshield.example.com"),
);

check(
  "canonical resolver rejects HTTP for an approved hostname",
  canonicalResolverRejects("http://www.subprecheck.com"),
);

check(
  "canonical resolver rejects embedded URL credentials",
  canonicalResolverRejects("https://user:pass@www.subprecheck.com"),
);
