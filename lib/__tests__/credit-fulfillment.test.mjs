...
} from "../stripe-subscription-invoice.ts";
...
check("active initial subscription invoice grants 3", initialGrant.kind === "eligible" && initialGrant.credits === 3);

const renewalGrant = await resolveSubscriptionInvoiceGrant(subscriptionStripe(), invoice(), activePlan);
check("active renewal invoice grants 3 and scans all invoice lines", renewalGrant.kind === "eligible" && renewalGrant.credits === 3);
...
check("safely expanded active subscription is used without another lookup", expandedGrant.kind === "eligible" && expandedGrant.credits === 3);
...
for (const status of ["canceled", "incomplete", "incomplete_expired", "unpaid", "paused", "past_due", "trialing"]) {
  ...
  check(`${status} subscription invoice grants nothing`, grant.kind === "ineligible" && grant.reason === "inactive_subscription");
}
...
check("nullable invoice email falls back to Stripe Customer", customerFallback.kind === "eligible" && customerFallback.email === "fallback@example.com");
...
check(
  "qualifying invoice without either email requires reconciliation",
  noEmailGrant.kind === "needs_reconciliation"
    && noEmailGrant.reason === "missing_stripe_email"
    && noEmailGrant.subscriptionId === "sub_test"
    && noEmailGrant.customerId === "cus_test",
);
check(
  "webhook policy treats reconciliation as retryable",
  subscriptionInvoiceResponseStatus(noEmailGrant) === 500,
);

const ordinaryIneligible = await resolveSubscriptionInvoiceGrant(
  subscriptionStripe(),
  invoice({ status: "open" }),
  activePlan,
);
check(
  "ordinary ineligible invoice is acknowledged",
  ordinaryIneligible.kind === "ineligible"
    && ordinaryIneligible.reason === "invoice_state"
    && subscriptionInvoiceResponseStatus(ordinaryIneligible) === 200,
);

const reconciliationDatabase = new MemoryCreditDatabase();
check(
  "missing-email resolution creates no credit or idempotency record",
  noEmailGrant.kind === "needs_reconciliation"
    && reconciliationDatabase.eventIds.size === 0
    && reconciliationDatabase.sourceIds.size === 0,
);
const correctedResolution = await resolveSubscriptionInvoiceGrant(
  subscriptionStripe({ customerEmail: "corrected@example.com" }),
  invoice({ id: "in_reconciled", customer_email: null }),
  activePlan,
);
const correctedGrant = correctedResolution.kind === "eligible"
  && await fulfillCheckoutCredits(reconciliationDatabase, {
    eventId: "evt_reconciled",
    sourceType: "invoice",
    sourceId: "in_reconciled",
    email: correctedResolution.email,
    credits: correctedResolution.credits,
  });
check(
  "resend after Stripe Customer email correction grants exactly once",
  correctedGrant === true && reconciliationDatabase.pending.get("corrected@example.com") === 3,
);
const duplicateCorrectedGrant = correctedResolution.kind === "eligible"
  && await fulfillCheckoutCredits(reconciliationDatabase, {
    eventId: "evt_reconciled_retry",
    sourceType: "invoice",
    sourceId: "in_reconciled",
    email: correctedResolution.email,
    credits: correctedResolution.credits,
...
check(
  "duplicate corrected resend remains idempotent",
  duplicateCorrectedGrant === false && reconciliationDatabase.pending.get("corrected@example.com") === 3,
);
