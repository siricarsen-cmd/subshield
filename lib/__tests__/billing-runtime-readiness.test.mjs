...
// node --experimental-strip-types lib/__tests__/billing-runtime-readiness.test.mjs
import assert from "node:assert/strict";
import { resolveAppBaseUrl } from "../app-base-url.ts";
import { requireStripeSecretKey } from "../stripe.ts";
import { requireStripeWebhookDatabaseEnv } from "../stripe-webhook-database.ts";
...
check("webhook database module import does not require service-role secrets", () => {
  assert.throws(
    () => requireStripeWebhookDatabaseEnv({}),
    /SUPABASE_URL/,
  );
});

check("webhook database validation requires the service-role key", () => {
  assert.throws(
    () => requireStripeWebhookDatabaseEnv({
...
check("webhook database validation normalizes a complete configuration", () => {
  assert.deepEqual(
    requireStripeWebhookDatabaseEnv({
