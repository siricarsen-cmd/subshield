...
} from "../affirmative-signals.ts";
import { verifyFindings, hasShortNoticeWaiverRiskEvidence } from "../sanity.ts";
import { dedupeFindings, rankFindings } from "../report.ts";
import { normalizeWhitespace, quoteExistsInDocument } from "../text.ts";

const IRONCLAD = `PROJECT IRONCLAD HORIZON
...
The awarded base scope does not require access to Controlled Unclassified Information (CUI), classified information, or Government production information systems. DFARS 252.204-7012 and NIST SP 800-171 are not incorporated into this subcontract for the awarded base scope. If the Government later requires CUI handling or a materially different cybersecurity baseline, the parties will execute a bilateral modification addressing requirements, schedule, and price before that changed work begins.
...
9. INSURANCE
Subcontractor shall maintain CGL limits of $2,000,000 per occurrence and $4,000,000 aggregate, automobile liability of $2,000,000, statutory workers compensation, employer liability of $1,000,000, umbrella/excess liability of $10,000,000, contractor pollution liability of $5,000,000, and professional liability of $2,000,000 when delegated design applies. These insurance requirements are included in the firm-fixed price.
...
11. TECHNICAL DATA AND PROPRIETARY RIGHTS
...
14. COMPLETE ATTACHMENTS AND FUTURE FLOWDOWNS
...
const ORION_CYBER =
  "Subcontractor shall comply with DFARS 252.204-7012 and NIST SP 800-171 for all systems processing Controlled Unclassified Information (CUI).";
const ORION_NOTICE =
  "Subcontractor must notify Prime Contractor in writing within 2 business days after receiving a direction that changes scope or schedule. Failure to provide timely notice waives Subcontractor's right to request additional compensation, equitable adjustment, or schedule relief.";
const ORION_CURE =
  "Prime Contractor may terminate for default if Subcontractor fails to cure within 5 calendar days after notice. Prime Contractor may terminate immediately without an opportunity to cure for a serious security violation.";
...
const documentText = normalizeWhitespace(IRONCLAD);
...
check("classification: Ironclad is FFP", classification.contractType === "FFP", classification.contractType);
check("classification: construction/facility wins", classification.sector === "Construction / Facility / Trade", classification.sector);
check(
  "classification: sector evidence is affirmative construction text",
  Boolean(classification.sectorEvidence && /field-install|construction|structural|facility/i.test(classification.sectorEvidence))
);
check(
  "classification: sector evidence is not cyber exclusion text",
  !/CUI|DFARS|NIST|does not require/i.test(classification.sectorEvidence ?? "")
);

for (const key of ["construction", "labor", "insurance", "supply", "audit", "data-rights"]) {
  check(`routing: ${key} family selected`, familyKeys.has(key), [...familyKeys].join(", "));
}
check("routing: negative-only cyber language does not select cyber family", !familyKeys.has("cyber"), [...familyKeys].join(", "));

check("cyber negative: Ironclad exclusion is not affirmative cyber evidence", !hasAffirmativeCyberSignal(documentText));
check("cyber positive: Orion affirmative cyber remains evidence", hasAffirmativeCyberSignal(ORION_CYBER));
check("labor positive: construction wage/certified payroll is affirmative", hasAffirmativeConstructionWageSignal(documentText));
check("SCLS negative: explicit non-applicability is not affirmative SCLS evidence", !hasAffirmativeSclsSignal(documentText));
check(
  "SCLS positive semantic control remains affirmative",
  hasAffirmativeSclsSignal(
    "Subcontractor shall comply with Service Contract Labor Standards under FAR 52.222-41 for covered service employees."
  )
);
...
for (const label of [
  ...
  check(`positive finding: ${label}`, labels.has(label), [...labels].join(" | "));
}
check(
  "grounding: every final Ironclad deterministic quote exists in source",
  finalFindings.every((finding) => quoteExistsInDocument(finding.foundText, documentText))
);

check(
  "negative finding: no affirmative DFARS/CUI/NIST cyber flowdown",
  !finalFindings.some(
    (finding) => finding.familyKey === "cyber" && /252\.204-7012|CUI|NIST\s*SP\s*800-171/i.test(finding.regulation)
  )
);
check("negative finding: no SCLS/SCA finding", !finalFindings.some((finding) => /SCLS|Service Contract Act|52\.222-41/i.test(finding.regulation)));
check("negative finding: no pay-if-paid", !finalFindings.some((finding) => /pay[\s-]if[\s-]paid|contingent government/i.test(finding.regulation)));
check("negative finding: no short change-notice waiver", !finalFindings.some((finding) => /Short Notice-of-Claim|Change Notice Waiver/i.test(finding.regulation)));
check("negative finding: Ironclad notice helper remains protective", !hasShortNoticeWaiverRiskEvidence(documentText));
check("positive control: Orion notice helper remains risky", hasShortNoticeWaiverRiskEvidence(ORION_NOTICE));
check("negative finding: no missing/deferred documents", !finalFindings.some((finding) => /Missing \/ Deferred|Missing or Deferred|Missing Documents/i.test(finding.regulation)));
check("negative finding: no unilateral future flowdown", !finalFindings.some((finding) => /Future Flowdowns|Prime Contract Control|Unilateral Future/i.test(finding.regulation)));
check("negative finding: no short/default-cure finding", !finalFindings.some((finding) => /Short Default Cure|Cure Period|Termination Discretion/i.test(finding.regulation)));

const negativeCyberModelFinding = {
  triggerType: "Contract Risk Trigger",
  regulation: "DFARS 252.204-7012 / CUI / NIST SP 800-171 Cybersecurity Flowdown",
  severity: "High",
  foundText: "DFARS 252.204-7012 and NIST SP 800-171 are not incorporated into this subcontract for the awarded base scope.",
  riskAnalysis: "This clause allegedly imposes DFARS and NIST requirements.",
  redlineFix: "Clarify cyber requirements.",
  familyKey: "cyber",
};
check(
  "cyber negative: simulated model false positive is rejected",
  verifyFindings([negativeCyberModelFinding], documentText).verified.length === 0
);

const negativeSclsModelFinding = {
  triggerType: "Regulatory Trigger",
  regulation: "Service Contract Labor Standards / SCA",
  severity: "Medium",
  foundText: "The Service Contract Labor Standards / Service Contract Act framework does not apply to the awarded base scope because the work is construction and installation.",
  riskAnalysis: "This clause allegedly imposes SCLS.",
  redlineFix: "Clarify labor standards.",
  familyKey: "labor",
};
check(
  "SCLS negative: simulated model false positive is rejected",
  verifyFindings([negativeSclsModelFinding], documentText).verified.length === 0
);

const simulatedGenericCure = {
  triggerType: "Contract Risk Trigger",
  regulation: "Liability, Indemnity, Disputes, Termination",
  severity: "Medium-High",
  foundText: "Either party may terminate for material breach that remains uncured for 30 calendar days after detailed written notice. If cure reasonably requires more than 30 days, the breaching party may continue cure under a written plan so long as it begins promptly and proceeds diligently.",
  riskAnalysis: "This clause allegedly provides too little cure time before termination.",
  redlineFix: "Extend the cure period.",
  familyKey: "liability",
};
check(
  "cure negative: generic model termination label is dropped for 30-day continuing cure",
  verifyFindings([simulatedGenericCure], documentText).verified.length === 0
);
const simulatedOrionCure = {
  ...simulatedGenericCure,
  regulation: "Short Default Cure Period / Termination Discretion",
  foundText: ORION_CURE,
};
check(
  "cure positive: Orion 5-day/immediate-termination model finding survives",
  verifyFindings([simulatedOrionCure], ORION_CURE).verified.length === 1
);

if (failures) {
  console.error(`\n${failures} Ironclad regression check(s) failed.`);
  process.exit(1);
}
console.log("\nAll Project Ironclad Horizon regression checks passed.");
