...
import {
  ...
  REAL_DUTY_TO_DEFEND_AGAINST_THIRD_PARTY_CLAIMS_CLAUSE,
  TERMINATION_PROHIBITED_WITHOUT_NOTICE_AND_CURE_CLAUSE,
  REAL_TERMINATION_IMMEDIATE_NO_NOTICE_CLAUSE,
  REAL_TERMINATION_SOLE_DISCRETION_CLAUSE,
  REAL_TERMINATION_NO_RIGHT_TO_CURE_CLAUSE,
  TERMINATION_PROHIBITED_WITH_EXPLICIT_EXCEPTION_CLAUSE,
...
// ---------------------------------------------------------------------------
// E. Protective "no duty to defend" guard
// ---------------------------------------------------------------------------
{
  const noDutyFinding = mkFinding({
    familyKey: "liability",
    regulation: "Broad Indemnification / Duty to Defend",
    foundText: INDEMNITY_NO_DUTY_TO_DEFEND_CLAUSE,
  });
  const { verified, dropped } = verifyFindings([noDutyFinding], INDEMNITY_NO_DUTY_TO_DEFEND_CLAUSE);
  check("E1. 'no duty to defend' finding is suppressed", verified.length === 0 && dropped.length === 1);
}
{
  const realIndemnityFinding = mkFinding({
    familyKey: "liability",
    regulation: "Broad Indemnification / Duty to Defend",
    foundText: REAL_INDEMNITY_CLAUSE,
  });
  const { verified, dropped } = verifyFindings([realIndemnityFinding], REAL_INDEMNITY_CLAUSE);
  check("E2. real indemnify/defend/hold-harmless finding survives", verified.length === 1 && dropped.length === 0);
}
{
  const realDutyFinding = mkFinding({
    familyKey: "liability",
    regulation: "Broad Indemnification / Duty to Defend",
    foundText: REAL_DUTY_TO_DEFEND_CLAUSE,
  });
  const { verified, dropped } = verifyFindings([realDutyFinding], REAL_DUTY_TO_DEFEND_CLAUSE);
  check("E3. real 'has a duty to defend' finding survives", verified.length === 1 && dropped.length === 0);
}
{
  // Not currently producible by the deterministic detector, but must not be
  // wrongly suppressed if an LLM raises it independently.
  const realDefendAgainstClaimsFinding = mkFinding({
    familyKey: "liability",
    regulation: "Broad Indemnification / Duty to Defend",
    foundText: REAL_DUTY_TO_DEFEND_AGAINST_THIRD_PARTY_CLAIMS_CLAUSE,
  });
  const { verified, dropped } = verifyFindings(
    [realDefendAgainstClaimsFinding],
    REAL_DUTY_TO_DEFEND_AGAINST_THIRD_PARTY_CLAIMS_CLAUSE
  );
  check(
    "E4. real 'shall defend...against third-party claims' finding survives",
    verified.length === 1 && dropped.length === 0
  );
}
{
  // Protective "no duty to defend" language AND a separate, explicit
  // affirmative indemnity exception in the SAME quote - must survive.
  const protectiveWithExceptionFinding = mkFinding({
    familyKey: "liability",
    regulation: "Broad Indemnification / Duty to Defend",
    foundText: INDEMNITY_PROTECTIVE_WITH_AFFIRMATIVE_EXCEPTION_CLAUSE,
  });
  const { verified, dropped } = verifyFindings(
    [protectiveWithExceptionFinding],
    INDEMNITY_PROTECTIVE_WITH_AFFIRMATIVE_EXCEPTION_CLAUSE
  );
  check(
    "E5. protective 'no duty to defend' + explicit affirmative indemnity exception in the same quote survives",
    verified.length === 1 && dropped.length === 0
  );
}

// ---------------------------------------------------------------------------
// F. Protective termination/cure prohibition guard
// ---------------------------------------------------------------------------
{
  const prohibitionFinding = mkFinding({
    familyKey: "liability",
    regulation: "Short Default Cure Period / Termination Discretion",
    foundText: TERMINATION_PROHIBITED_WITHOUT_NOTICE_AND_CURE_CLAUSE,
  });
  const { verified, dropped } = verifyFindings(
    [prohibitionFinding],
    TERMINATION_PROHIBITED_WITHOUT_NOTICE_AND_CURE_CLAUSE
  ...
  check(
    "F1. 'neither party may terminate...without notice and cure' finding is suppressed",
    verified.length === 0 && dropped.length === 1
  );
}
{
  const realImmediateFinding = mkFinding({
    familyKey: "liability",
    regulation: "Short Default Cure Period / Termination Discretion",
    foundText: REAL_TERMINATION_IMMEDIATE_NO_NOTICE_CLAUSE,
  });
  const { verified, dropped } = verifyFindings([realImmediateFinding], REAL_TERMINATION_IMMEDIATE_NO_NOTICE_CLAUSE);
  check(
    "F2. real 'Prime Contractor may terminate immediately without notice' finding survives",
    verified.length === 1 && dropped.length === 0
  );
}
{
  const realSoleDiscretionFinding = mkFinding({
    familyKey: "liability",
    regulation: "Short Default Cure Period / Termination Discretion",
    foundText: REAL_TERMINATION_SOLE_DISCRETION_CLAUSE,
  });
  const { verified, dropped } = verifyFindings([realSoleDiscretionFinding], REAL_TERMINATION_SOLE_DISCRETION_CLAUSE);
  check(
    "F3. real 'Prime Contractor may terminate in its sole discretion' finding survives",
    verified.length === 1 && dropped.length === 0
  );
}
{
  const realNoRightToCureFinding = mkFinding({
    familyKey: "liability",
    regulation: "Short Default Cure Period / Termination Discretion",
    foundText: REAL_TERMINATION_NO_RIGHT_TO_CURE_CLAUSE,
  });
  const { verified, dropped } = verifyFindings([realNoRightToCureFinding], REAL_TERMINATION_NO_RIGHT_TO_CURE_CLAUSE);
  check(
    "F4. real 'Prime Contractor may terminate without any right or opportunity to cure' finding survives",
    verified.length === 1 && dropped.length === 0
  );
}
{
  // Protective prohibition AND a separate explicit exception granting Prime
  // an immediate/no-notice right, in the SAME quote - must survive.
  const prohibitionWithExceptionFinding = mkFinding({
    familyKey: "liability",
    regulation: "Short Default Cure Period / Termination Discretion",
    foundText: TERMINATION_PROHIBITED_WITH_EXPLICIT_EXCEPTION_CLAUSE,
  });
  const { verified, dropped } = verifyFindings(
    [prohibitionWithExceptionFinding],
    TERMINATION_PROHIBITED_WITH_EXPLICIT_EXCEPTION_CLAUSE
  );
  check(
    "F5. prohibition + explicit Prime immediate-termination exception in the same quote survives",
    verified.length === 1 && dropped.length === 0
  );
}

// ---------------------------------------------------------------------------
// G. Complete clean Preview regression text
// ---------------------------------------------------------------------------
{
  // The exact protective quotes from the complete clean text, verified
  // against the full document - proves quote-grounding still holds and all
  // relevant guards still suppress when checked against the fuller context.
  const net20Finding = mkFinding({
    familyKey: "payment",
    regulation: "Ordinary Invoice Timing",
    foundText: "Prime Contractor will pay each correct invoice within 20 calendar days after receipt.",
  ...
  const { verified, dropped } = verifyFindings(
    [net20Finding, noDutyFinding, prohibitionFinding],
    COMPLETE_CLEAN_PREVIEW_CONTRACT
  );
  check(
    "G1. ordinary Net-20, no-duty-to-defend, and termination-prohibition mock findings from the complete clean text are all suppressed",
    verified.length === 0 && dropped.length === 3
  );
}
