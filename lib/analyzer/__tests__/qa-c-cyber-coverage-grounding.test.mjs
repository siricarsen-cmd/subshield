...
import {
  ...
  CYBER_AUDIT_EVIDENCE_WITH_PRODUCTION_FOOTER,
  CYBER_REMEDIATION_CONTEXT_CLAUSE,
  BROAD_CYBER_INDEMNITY_WITH_HEADING_CLAUSE,
  ...
  FIXTURE_C_REPORT_125_EXTRACTED_TEXT,
  FIXTURE_C_REPORT_125_FOOTER_VARIANTS,
  FIXTURE_C_SINGLE_NEWLINE_DOCUMENT,
  FIXTURE_C_TWO_NEARBY_PAGE_FOOTERS_DOCUMENT,
  ...
  IMMEDIATE_NO_CURE_TERMINATION_WITH_HEADING_CLAUSE,
...
// Literal report-125 extraction shape plus model/deterministic collision
// coverage. These tests never call the model: each model object is a locally
// constructed, exactly grounded Finding that reproduces the observed outer
// heading/footer contamination.
{
  const result = runProductionPath(FIXTURE_C_REPORT_125_EXTRACTED_TEXT);
  const deterministicByLabel = new Map(
    result.deterministic.map((finding) => [finding.regulation, finding])
  );
  const finalByLabel = new Map(
    result.finalFindings.map((finding) => [finding.regulation, finding])
  );
  const baseline = deterministicByLabel.get(LABELS.baseline);
  const indemnity = deterministicByLabel.get(LABELS.indemnity);
  const noCure = deterministicByLabel.get(LABELS.noCure);
  const access = deterministicByLabel.get(LABELS.auditAccess);

  check("G1. report-125 deterministic DFARS/NIST quote starts at 2.1", baseline?.foundText.startsWith("2.1 "));
  check("G2. report-125 deterministic indemnity quote starts at 9.1", indemnity?.foundText.startsWith("9.1 "));
  check("G3. report-125 deterministic no-cure quote starts at 10.1", noCure?.foundText.startsWith("10.1 "));
  check(
    "G4. report-125 deterministic 5.2 quote is clause-local and footer-free",
    access?.foundText.startsWith("5.2 ") &&
      access.foundText.endsWith("without additional charge.") &&
      !/-- 1 of 2 --|QA-C|Fictional Test Document|Page 2/i.test(access.foundText)
  );
  check(
    "G5. report-125 shape preserves all 16 Fixture C identities",
    result.finalFindings.length === 16 &&
      JSON.stringify(result.finalFindings.map((finding) => finding.regulation).sort()) ===
        JSON.stringify(REQUIRED_LABELS)
  );
  ...
  const affectedCases = [
    {
      ...
      contaminatedQuote: DFARS_NIST_CYBER_CLAUSE,
    },
    {
      name: "indemnity",
      label: LABELS.indemnity,
      deterministic: indemnity,
      contaminatedQuote: BROAD_CYBER_INDEMNITY_WITH_HEADING_CLAUSE,
    },
    {
      name: "no-cure termination",
      label: LABELS.noCure,
      deterministic: noCure,
      contaminatedQuote: IMMEDIATE_NO_CURE_TERMINATION_WITH_HEADING_CLAUSE,
    },
    {
      name: "access evidence",
      label: LABELS.auditAccess,
      deterministic: access,
      contaminatedQuote: CYBER_AUDIT_EVIDENCE_WITH_PRODUCTION_FOOTER,
    },
  ].filter((entry) => Boolean(entry.deterministic));

  const severityScenarios = [
    {
      name: "equal severity",
      modelSeverity: "Medium-High",
      deterministicSeverity: "Medium-High",
      expectedWinner: "model",
    },
    {
      name: "model higher severity",
      modelSeverity: "High",
      deterministicSeverity: "Medium",
      expectedWinner: "model",
    },
    {
      name: "model lower severity",
      modelSeverity: "Medium",
      deterministicSeverity: "High",
      expectedWinner: "deterministic",
    },
  ];

  function runCollisionMatrix(entry, modelFamilyKey, familyCaseName) {
    for (const severityCase of severityScenarios) {
      const deterministicCandidate = {
        ...entry.deterministic,
        severity: severityCase.deterministicSeverity,
        triggerType: "Contract Risk Trigger",
        redlineFix: `DETERMINISTIC METADATA: ${entry.name} / ${severityCase.name}`,
      };
      const modelCandidate = {
        ...entry.deterministic,
        familyKey: modelFamilyKey,
        severity: severityCase.modelSeverity,
        triggerType: "Regulatory Trigger",
        foundText: entry.contaminatedQuote,
        redlineFix: `MODEL METADATA: ${entry.name} / ${severityCase.name}`,
      };
      const expectedMetadata =
        severityCase.expectedWinner === "model"
          ? modelCandidate
          : deterministicCandidate;
      const { verified, dropped } = verifyFindings(
        [modelCandidate, deterministicCandidate],
        result.documentText
      );

      check(
        `G6 ${entry.name} / ${familyCaseName} / ${severityCase.name}: both candidates verify before dedupe`,
        verified.length === 2 && dropped.length === 0
      );

      for (const [orderName, orderedFindings] of [
        ["model-first", [modelCandidate, deterministicCandidate]],
        ["deterministic-first", [deterministicCandidate, modelCandidate]],
      ]) {
        const { verified: orderedVerified } = verifyFindings(
          orderedFindings,
          result.documentText
        );
        const deduped = dedupeFindings(orderedVerified, result.documentText);
        const [survivor] = deduped;

        check(
          `G7 ${entry.name} / ${familyCaseName} / ${severityCase.name} / ${orderName}: exactly one same-risk finding survives`,
          deduped.length === 1 && survivor?.regulation === entry.label
        );
        check(
          `G8 ${entry.name} / ${familyCaseName} / ${severityCase.name} / ${orderName}: severity metadata winner is preserved`,
          survivor?.severity === expectedMetadata.severity &&
            survivor?.redlineFix === expectedMetadata.redlineFix &&
            survivor?.triggerType === expectedMetadata.triggerType &&
            survivor?.familyKey === expectedMetadata.familyKey
        );
        check(
          `G9 ${entry.name} / ${familyCaseName} / ${severityCase.name} / ${orderName}: clean deterministic evidence quote is selected`,
          survivor?.foundText === entry.deterministic.foundText
        );
        check(
          `G10 ${entry.name} / ${familyCaseName} / ${severityCase.name} / ${orderName}: merged object re-verifies against complete report-125 text`,
          Boolean(
            survivor &&
              verifyFindings([survivor], result.documentText).verified.length === 1
          )
        );
      }
    }
  ...
  for (const entry of affectedCases) {
    runCollisionMatrix(entry, entry.deterministic.familyKey, "matching family");
  }
  if (access) {
    runCollisionMatrix(
      affectedCases.find((entry) => entry.label === LABELS.auditAccess),
      "cyber",
      "audit/cyber family mismatch"
    );
  }

  check(
    "G11. strict adornment helper recognizes a leading report-125 heading",
    Boolean(
      baseline &&
        hasStrictOuterAdornmentRelationship(
          baseline.foundText,
  ...
  check(
    "G12. strict adornment helper recognizes the production footer",
    Boolean(
      access &&
        hasStrictOuterAdornmentRelationship(
          access.foundText,
  ...
  check(
    "G13. strict adornment helper rejects arbitrary uppercase operative prose",
    Boolean(
      access &&
        !hasStrictOuterAdornmentRelationship(
          access.foundText,
          `SUBCONTRACTOR SHALL PROVIDE ${access.foundText}`
        )
    )
  );
  check(
    "G14. strict adornment helper rejects ordinary contractual Page wording",
    Boolean(
      access &&
        !hasStrictOuterAdornmentRelationship(
          access.foundText,
          `${access.foundText} Page 2 describes the delivery schedule.`
        )
    )
  );
  check(
    "G15. strict adornment helper rejects mismatched numbered headings",
    Boolean(
      baseline &&
        !hasStrictOuterAdornmentRelationship(
          baseline.foundText,
          `9. REQUIRED CYBER FRAMEWORKS ${baseline.foundText}`
        )
    )
  );
  check(
    "G16. strict adornment helper rejects a FAR/DFARS citation as a heading",
    Boolean(
      baseline &&
        !hasStrictOuterAdornmentRelationship(
          baseline.foundText,
          `DFARS 252.204-7012 ${baseline.foundText}`
        )
    )
  );

  const modelOnly = {
    ...deterministicByLabel.get(LABELS.auditAssessment),
    regulation: "Model-Only Scoped Cyber Assessment Finding",
    triggerType: "Regulatory Trigger",
  };
  const { verified: verifiedModelOnly } = verifyFindings(
    [modelOnly],
    result.documentText
  );
  const [modelOnlyAfterDedupe] = dedupeFindings(
    verifiedModelOnly,
    result.documentText
  );
  check(
    "G17. a model-only finding remains unchanged",
    JSON.stringify(modelOnlyAfterDedupe) === JSON.stringify(modelOnly)
  );

  const distinct51And52 = [
    deterministicByLabel.get(LABELS.auditAssessment),
    deterministicByLabel.get(LABELS.auditAccess),
  ].filter(Boolean);
  const distinct91And93 = [
    deterministicByLabel.get(LABELS.indemnity),
    deterministicByLabel.get(LABELS.uncapped),
  ].filter(Boolean);
  const distinctTermination = [
    deterministicByLabel.get(LABELS.noCure),
    deterministicByLabel.get(LABELS.convenience),
    deterministicByLabel.get(LABELS.selfFinancedRemediation),
  ].filter(Boolean);
  check(
    "G18. sections 5.1 and 5.2 remain distinct after document-aware dedupe",
    dedupeFindings(distinct51And52, result.documentText).length === 2
  );
  check(
    "G19. sections 9.1 and 9.3 remain distinct after document-aware dedupe",
    dedupeFindings(distinct91And93, result.documentText).length === 2
  );
  check(
    "G20. sections 10.1, 10.2, and 10.3 remain distinct after document-aware dedupe",
    dedupeFindings(distinctTermination, result.documentText).length === 3
  );

  const remediation = finalByLabel.get(LABELS.remediation);
  check(
    "G21. report-125 sections 5.3-5.4 remain one permitted contiguous passage",
    Boolean(
      remediation &&
        remediation.foundText.startsWith("5.3 ") &&
        /5\.4 A suspension or remediation directive/i.test(remediation.foundText) &&
  ...
  const cleanMemo = access ? buildPmMemo([access], [], false) : "";
  const adornedMemo = access
    ? buildPmMemo(
        [{ ...access, foundText: CYBER_AUDIT_EVIDENCE_WITH_PRODUCTION_FOOTER }],
        [],
        false
      )
    : "different";
  check(
    "G22. PM memo output is unchanged when only foundText cleanliness differs",
    cleanMemo === adornedMemo
  );

  const limitedScan = await runAnalyzer(
    "Short local text that cannot satisfy extraction confidence.",
    "fixture-c-limited-scan.txt"
  );
  check(
    "G23. Limited Scan behavior remains unchanged without invoking a model",
    limitedScan.limitedScan === true &&
      limitedScan.primaryTraps.length === 0 &&
      limitedScan.secondaryConcerns.length === 0 &&
      limitedScan.emailDraft === ""
  );
}

// Complete production-shaped multiline footer variants. These exercise the
// source-position boundary finder, clause segmentation, deterministic recall,
// exact grounding, dedupe, and final identity preservation together.
for (const variant of FIXTURE_C_REPORT_125_FOOTER_VARIANTS) {
  const result = runProductionPath(variant.documentText);
  const segments = extractClauseSegments(result.documentText);
  const section52 = segments.find((segment) => segment.number === "5.2");
  const section53 = segments.find((segment) => segment.number === "5.3");
  const deterministicAccess = result.deterministic.find(
    (finding) => finding.regulation === LABELS.auditAccess
  );
  const finalByLabel = new Map(
    result.finalFindings.map((finding) => [finding.regulation, finding])
  );
  const finalAssessment = finalByLabel.get(LABELS.auditAssessment);
  const finalAccess = finalByLabel.get(LABELS.auditAccess);
  const remediation = finalByLabel.get(LABELS.remediation);
  const response = finalByLabel.get(LABELS.response);
  const missing = finalByLabel.get(LABELS.missing);
  const footerFree =
    section52?.text.startsWith("5.2 ") &&
    section52.text.endsWith("without additional charge.") &&
    !/-- 1 of 2 --|QA-C|Fictional Test Document|Page 2/i.test(section52.text);

  check(
    `H1 ${variant.name}: section 5.2 segment is exact and footer-free`,
    Boolean(footerFree)
  );
  check(
    `H2 ${variant.name}: deterministic access finding uses the exact clean segment`,
    Boolean(
      section52 &&
        deterministicAccess &&
        deterministicAccess.foundText === section52.text
    )
  );
  check(
    `H3 ${variant.name}: clean access quote verifies against the complete document`,
    Boolean(
      deterministicAccess &&
        quoteExistsInDocument(deterministicAccess.foundText, result.documentText)
    )
  );
  check(
    `H4 ${variant.name}: section 5.3 remains a separately extracted clause`,
    Boolean(
      section53?.text.startsWith("5.3 ") &&
        !/-- 1 of 2 --|QA-C|Fictional Test Document|Page 2/i.test(section53.text)
    )
  );
  check(
    `H5 ${variant.name}: sections 5.1 and 5.2 remain distinct findings`,
    Boolean(
      finalAssessment &&
        finalAccess &&
        finalAssessment.foundText.startsWith("5.1 ") &&
        finalAccess.foundText.startsWith("5.2 ") &&
        finalAssessment.foundText !== finalAccess.foundText
    )
  );
  check(
    `H6 ${variant.name}: sections 5.3-5.4 remain one grounded combined passage`,
    Boolean(
      remediation &&
        remediation.foundText.startsWith("5.3 ") &&
        /5\.4 A suspension or remediation directive/i.test(remediation.foundText) &&
  ...
  check(
    `H7 ${variant.name}: all 16 Fixture C identities remain`,
    result.finalFindings.length === REQUIRED_LABELS.length &&
      JSON.stringify(
        result.finalFindings.map((finding) => finding.regulation).sort()
      ) === JSON.stringify(REQUIRED_LABELS)
  );
  check(
    `H8 ${variant.name}: repaired footer boundaries preserve unrelated controlled findings`,
    Boolean(
      response?.foundText.startsWith("4.4 ") &&
        /bear all associated cost\.$/i.test(response.foundText) &&
        missing &&
        /not attached at execution/i.test(missing.foundText) &&
        /after award/i.test(missing.foundText) &&
        finalByLabel.get(LABELS.baseline)?.foundText.startsWith("2.1 ") &&
        finalByLabel.get(LABELS.indemnity)?.foundText.startsWith("9.1 ") &&
        finalByLabel.get(LABELS.noCure)?.foundText.startsWith("10.1 ")
    )
  );
}

// Both strict counters can match the terminal Page 3 suffix, so the boundary
// finder must keep scanning and return the later original-source position.
{
  const documentText = FIXTURE_C_TWO_NEARBY_PAGE_FOOTERS_DOCUMENT;
  const segments = extractClauseSegments(documentText);
  const section51 = segments.find((segment) => segment.number === "5.1");
  const section52 = segments.find((segment) => segment.number === "5.2");
  const section53 = segments.find((segment) => segment.number === "5.3");
  const firstCounterStart = documentText.indexOf("-- 1 of 3 --");
  const secondCounterStart = documentText.indexOf("-- 2 of 3 --");
  const section52Start = documentText.indexOf("5.2 Subcontractor");
  const section53Start = documentText.indexOf("5.3 Prime");
  const strictFooterSuffixRe =
    /^--+\s*\d+\s+of\s+\d+\s*--+[\s\S]{0,180}\|\s*Page\s+\d+[ \t\r\n\f]*$/i;

  check(
    "H9 nearby counters: both candidates satisfy the bounded strict terminal-footer grammar",
    strictFooterSuffixRe.test(documentText.slice(firstCounterStart, section53Start)) &&
      strictFooterSuffixRe.test(documentText.slice(secondCounterStart, section53Start))
  );
  check(
    "H10 nearby counters: section 5.1 excludes its footer",
    section51?.text === "5.1 A short operative clause ends here."
  );
  check(
    "H11 nearby counters: the first candidate does not consume or interfere with section 5.2",
    section51?.end === firstCounterStart &&
      section52?.start === section52Start &&
      section52.start > firstCounterStart
  );
  check(
    "H12 nearby counters: section 5.2 retains its complete operative sentence",
    section52?.text ===
      "5.2 Subcontractor shall provide administrative access and evidence without additional charge."
  );
  check(
    "H13 nearby counters: section 5.2 excludes the second footer",
    Boolean(
      section52 &&
        !/-- 2 of 3 --|QA-C|Fictional Test Document|Page 3/i.test(section52.text)
    )
  );
  check(
    "H14 nearby counters: suffix selection uses the latest valid original-source counter",
    section52?.end === secondCounterStart && section52.end !== firstCounterStart
  );
  check(
    "H15 nearby counters: section 5.3 remains a separate clause",
    section53?.start === section53Start &&
      section53.text === "5.3 Prime may reject a control implementation."
  );
}

// A production-realistic model candidate is intentionally constructed field by
// field rather than inheriting deterministic metadata. This proves the clean
// evidence preference does not discard independently generated model analysis.
{
  const result = runProductionPath(FIXTURE_C_REPORT_125_EXTRACTED_TEXT);
  const deterministicAccess = result.deterministic.find(
    (finding) => finding.regulation === LABELS.auditAccess
  ...
  const severityScenarios = [
    {
      name: "equal model severity",
      modelSeverity: "Medium-High",
      deterministicSeverity: "Medium-High",
      expectedWinner: "model",
    },
    {
      name: "higher model severity",
      modelSeverity: "High",
      deterministicSeverity: "Medium-High",
      expectedWinner: "model",
    },
    {
      name: "lower model severity",
      modelSeverity: "Medium",
      deterministicSeverity: "High",
      expectedWinner: "deterministic",
    },
  ...
  for (const severityCase of severityScenarios) {
    const deterministicCandidate = deterministicAccess
      ? {
          ...deterministicAccess,
          severity: severityCase.deterministicSeverity,
        }
    ...
    const modelCandidate = {
      ...
      severity: severityCase.modelSeverity,
      foundText: CYBER_AUDIT_EVIDENCE_WITH_PRODUCTION_FOOTER,
      riskAnalysis:
        "This clause requires administrative access and broad cybersecurity evidence production without additional charge, creating security, confidentiality, operational, and uncompensated support exposure.",
      redlineFix:
        "Limit administrative access and evidence requests to records reasonably necessary for a defined assessment, require advance notice, least-privilege and confidentiality controls, and compensate material out-of-scope support.",
      familyKey: "cyber",
    };
    const expectedMetadata =
      severityCase.expectedWinner === "model"
    ...
    const initialVerification = verifyFindings(
      [modelCandidate, deterministicCandidate].filter(Boolean),
      result.documentText
    );
    check(
      `I1 ${severityCase.name}: independent model and deterministic candidates both verify`,
      initialVerification.verified.length === 2 &&
        initialVerification.dropped.length === 0
    );

    for (const [orderName, orderedCandidates] of [
      ["model-first", [modelCandidate, deterministicCandidate]],
      ["deterministic-first", [deterministicCandidate, modelCandidate]],
    ]) {
      const { verified } = verifyFindings(
      ...
      const deduped = dedupeFindings(verified, result.documentText);
      const [survivor] = deduped;

      check(
        `I2 ${severityCase.name} / ${orderName}: collision yields exactly one finding`,
        deduped.length === 1 &&
          survivor?.regulation ===
            "Broad Cybersecurity System Access / Evidence Production"
      );
      check(
        `I3 ${severityCase.name} / ${orderName}: complete metadata winner survives`,
        Boolean(
          survivor &&
            expectedMetadata &&
            survivor.regulation === expectedMetadata.regulation &&
            survivor.riskAnalysis === expectedMetadata.riskAnalysis &&
            survivor.redlineFix === expectedMetadata.redlineFix &&
            survivor.familyKey === expectedMetadata.familyKey &&
            survivor.triggerType === expectedMetadata.triggerType &&
            survivor.severity === expectedMetadata.severity
      ...
      check(
        `I4 ${severityCase.name} / ${orderName}: clean deterministic foundText survives`,
        Boolean(
          survivor &&
            deterministicAccess &&
            survivor.foundText === deterministicAccess.foundText &&
            survivor.foundText.endsWith("without additional charge.") &&
            !/-- 1 of 2 --|QA-C|Fictional Test Document|Page 2/i.test(
              survivor.foundText
            )
        )
      );
      check(
        `I5 ${severityCase.name} / ${orderName}: merged result re-verifies`,
        Boolean(
          survivor &&
            verifyFindings([survivor], result.documentText).verified.length === 1
        )
...
// Strict negative controls: none has the complete terminal, bounded,
// punctuation-preceded "-- N of M -- ... | Page N" suffix shape.
{
  const accessWithoutFinalPeriod = CYBER_AUDIT_EVIDENCE_CLAUSE.slice(0, -1);
  const nextClause = "5.3 Prime may reject requested evidence.";
  const longFooterBody = "X".repeat(181);
  const controls = [
    {
      name: "ordinary contractual Page 2 wording",
      documentText: `${CYBER_AUDIT_EVIDENCE_CLAUSE} Page 2 describes the delivery schedule and remains part of this Subcontract.\n${nextClause}`,
      retainedText: "Page 2 describes the delivery schedule",
    },
    {
      name: "undelimited 1 of 2 counter",
      documentText: `${CYBER_AUDIT_EVIDENCE_CLAUSE}\n1 of 2 QA-C | Fictional Test Document | Page 2\n${nextClause}`,
      retainedText: "1 of 2 QA-C",
    },
    {
      name: "counter without terminal Page 2",
      documentText: `${CYBER_AUDIT_EVIDENCE_CLAUSE}\n-- 1 of 2 -- QA-C | Fictional Test Document\n${nextClause}`,
      retainedText: "-- 1 of 2 --",
    },
    {
      name: "footer-like passage without preceding punctuation",
      documentText: `${accessWithoutFinalPeriod}\n-- 1 of 2 -- QA-C | Fictional Test Document | Page 2\n${nextClause}`,
      retainedText: "-- 1 of 2 --",
    },
    {
      name: "footer-like passage beyond the body limit",
      documentText: `${CYBER_AUDIT_EVIDENCE_CLAUSE}\n-- 1 of 2 -- ${longFooterBody}| Page 2\n${nextClause}`,
      retainedText: longFooterBody,
    },
    {
      name: "footer-like passage followed by operative prose",
      documentText: `${CYBER_AUDIT_EVIDENCE_CLAUSE}\n-- 1 of 2 -- QA-C | Fictional Test Document | Page 2\nSubcontractor shall preserve the requested records.\n${nextClause}`,
      retainedText: "Subcontractor shall preserve the requested records.",
    },
    {
      name: "unrelated QA-A and QA-B clause content",
      documentText: `${CYBER_AUDIT_EVIDENCE_CLAUSE} QA-A and QA-B requirements remain operative on Page 2.\n${nextClause}`,
      retainedText: "QA-A and QA-B requirements remain operative",
    },
  ];

  for (const control of controls) {
    const normalized = normalizeWhitespace(control.documentText);
    const section52 = extractClauseSegments(normalized).find(
      (segment) => segment.number === "5.2"
    );
    check(
      `J ${control.name}: strict recognizer retains non-footer content`,
      Boolean(section52?.text.includes(control.retainedText))
    );
  }
}
