...
const REPORT_125_SINGLE_LINE_FOOTER =
  "-- 1 of 2 -- QA-C | Fictional Test Document | Page 2";

function withReport125Footer(footer) {
  return FIXTURE_C_REPORT_125_EXTRACTED_TEXT.replace(
    REPORT_125_SINGLE_LINE_FOOTER,
    footer
  );
}

// Complete production-shaped documents for strict PDF-footer boundary tests.
// Each retains every controlled Fixture C clause so segmentation, deterministic
// detection, grounding, dedupe, and identity preservation run together.
export const FIXTURE_C_REPORT_125_FOOTER_VARIANTS = [
  {
    name: "single-line footer",
    documentText: FIXTURE_C_REPORT_125_EXTRACTED_TEXT,
  },
  {
    name: "page-count delimiter on its own line",
    documentText: withReport125Footer(
      "-- 1 of 2 --\nQA-C | Fictional Test Document | Page 2"
    ),
  },
  {
    name: "newline before first pipe",
    documentText: withReport125Footer(
      "-- 1 of 2 -- QA-C\n| Fictional Test Document | Page 2"
    ),
  },
  {
    name: "fully split footer",
    documentText: withReport125Footer(
      "-- 1 of 2 --\nQA-C\n| Fictional Test Document\n| Page 2"
    ),
  },
  {
    name: "form-feed before visible page header",
    documentText: withReport125Footer(
      "-- 1 of 2 --\n\f\nQA-C | Fictional Test Document | Page 2"
    ),
  },
  {
    name: "pdf-parse blank-line page joiner",
    documentText: withReport125Footer(
      "\n-- 1 of 2 --\n\nQA-C | Fictional Test Document | Page 2"
    ),
  },
...
// Both counters are within the 500-character boundary lookback, and the
// candidate beginning at the first counter has exactly 179 characters between
// its closing delimiter and the terminal "| Page 3" suffix.
export const FIXTURE_C_TWO_NEARBY_PAGE_FOOTERS_DOCUMENT = `5.1 A short operative clause ends here.
-- 1 of 3 -- QA-C | Fictional Test Document | Page 2
5.2 Subcontractor shall provide administrative access and evidence without additional charge.
-- 2 of 3 -- QA-C | Fictional Test Document | Page 3
5.3 Prime may reject a control implementation.`;
