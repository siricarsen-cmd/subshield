...
function unsupportedFindingLocalClaim(finding: Finding): string | null {
  ...
  const explicitForumSelectionClaim =
    /\bforum\s+(?:far|stated|required|selected)\b|(?:requires?|must|shall|required\s+to|permits?)[^.]{0,80}\b(?:disputes?|actions?|lawsuits?|claims?|proceedings?)\b[^.]{0,100}\b(?:litigat(?:e|ed|ion)|brought)\b|\b(?:imposes?|establishes?|mandates?|requires?|selects?|sets?)\s+(?:(?:an?|the)\s+)?(?:(?:exclusive|mandatory)\s+)?(?:venue|forum|jurisdiction)\b|\b(?:exclusive|mandatory)\s+(?:venue|forum|jurisdiction)\s+(?:is|shall\s+be|will\s+be|must\s+be)\b|\b(?:exclusive|mandatory)\s+(?:venue|forum|jurisdiction)\b[^.]{0,100}\b(?:in|at|within|required|selected|shall|must|will)\b|\b(?:exclusive\s+)?(?:venue|jurisdiction)\b[^.]{0,100}\b(?:required|selected|shall|must|will)\b/i.test(affirmativeClaim) ||
    filedInForumClaim;
