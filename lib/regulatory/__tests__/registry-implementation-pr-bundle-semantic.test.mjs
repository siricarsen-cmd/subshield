...
checkRejects(
  "deterministic metadata refuses a caller-reconstructed plan",
  () => buildRegulatoryImplementationPullRequestMetadata(unauthorizedPlan),
  /Implementation PR bundle plan is invalid/i
);
const unauthorizedBundle = structuredClone(bundle);
unauthorizedBundle.planChecksum = unauthorizedPlan.planChecksum;
unauthorizedBundle.bundleId =
  `regulatory-implementation-pr:${unauthorizedPlan.sourceId}:${unauthorizedPlan.planChecksum}`;
