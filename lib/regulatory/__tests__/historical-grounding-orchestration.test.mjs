...
check(
  ...
  overriddenPolicy.status === "invalid-policy" &&
    overriddenPolicy.refusalReasons.some((reason) =>
      /historical-policy differs from the immutable registry/i.test(reason)
    )
);
...
check(
  ...
  invalidPolicy.status === "invalid-policy" &&
    invalidPolicy.refusalReasons.some((reason) =>
      /historical-policy differs from the immutable registry/i.test(reason)
    )
);
