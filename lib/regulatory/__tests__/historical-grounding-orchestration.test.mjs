...
function exactExcerpt(sourceId) {
  return `Synthetic exact excerpt for ${sourceId}.`;
}
...
function citationPackage(mapping, selectedBySource) {
  return {
    ...
    citations: mapping.sourceComparisons.map((comparison) => {
      ...
      const excerpt = exactExcerpt(selected.sourceId);
      return {
...
const registeredPolicy = REGULATORY_HISTORICAL_GROUNDING_POLICIES.find(
  (policy) => policy.mappingId === mapping.mappingId
);
if (!registeredPolicy) throw new Error("Missing registered CMMC historical policy");
...
check(
  ...
  mismatch.status === "citation-package-mismatch" &&
    ...
    mismatch.refusalReasons.some((reason) => /snapshot ID.*7025|checksum.*7025/i.test(reason))
);

const provenanceTamperedPackage = citationPackage(mapping, bySource);
provenanceTamperedPackage.citations = provenanceTamperedPackage.citations.map((citation) =>
  citation.sourceId === "dfars-252-204-7025"
    ? {
        ...citation,
        versionIdentifier: histories["dfars-252-204-7025"][1].versionIdentifier,
        checksum: histories["dfars-252-204-7025"][1].checksum,
      }
    : citation
);
const provenanceTampered = orchestrateHistoricalRegulatoryGrounding({
  mapping,
  documentText: DOCUMENT,
  sourceHistories: histories,
  citationPackage: provenanceTamperedPackage,
});
check(
  "a citation cannot retain the selected ID while carrying another version's provenance",
  provenanceTampered.status === "citation-package-mismatch" &&
    provenanceTampered.refusalReasons.some(
      (reason) => /version identifier.*7025|snapshot checksum.*7025/i.test(reason)
    )
);

const foreignExcerptPackage = citationPackage(mapping, bySource);
foreignExcerptPackage.citations = foreignExcerptPackage.citations.map((citation) => {
  if (citation.sourceId !== "dfars-252-204-7025") return citation;
  const excerpt = "Foreign excerpt copied from another source snapshot.";
  return { ...citation, excerpt, excerptChecksum: sha256(excerpt) };
});
const foreignExcerpt = orchestrateHistoricalRegulatoryGrounding({
  mapping,
  documentText: DOCUMENT,
  sourceHistories: histories,
  citationPackage: foreignExcerptPackage,
});
check(
  "a citation excerpt must occur exactly in the selected immutable snapshot",
  foreignExcerpt.status === "citation-package-mismatch" &&
    foreignExcerpt.refusalReasons.some((reason) => /excerpt is not an exact substring.*7025/i.test(reason))
);
...
const alteredDatePolicy = {
  ...registeredPolicy,
  sourcePolicies: registeredPolicy.sourcePolicies.map((sourcePolicy) =>
    sourcePolicy.sourceId === "dfars-252-204-7025"
      ? { ...sourcePolicy, dateBasis: "subcontract-executed" }
      : { ...sourcePolicy }
  ),
};
const overriddenPolicy = orchestrateHistoricalRegulatoryGrounding({
  mapping,
  documentText: DOCUMENT,
  sourceHistories: histories,
  citationPackage: citationPackage(mapping, bySource),
  policy: alteredDatePolicy,
});
check(
  "a caller cannot override the registered solicitation date policy with execution",
  overriddenPolicy.status === "invalid-policy" &&
    overriddenPolicy.refusalReasons.some((reason) => /date basis differs.*7025/i.test(reason))
);
...
check(
  ...
  invalidPolicy.status === "invalid-policy" &&
    invalidPolicy.refusalReasons.some((reason) => /mapping differs|mapping mismatch/i.test(reason))
);
