...
const OLDER_BASE_COMMIT = "a7814406fdcc8fcd81eb5c12abc136e238c421f9";
...
try {
  ...
  const olderReviewRecord = buildStoredRegulatoryChangeSetReviewRecord(
    ...
    draftReceipt,
    {
      ...
      reviewedAt: "2026-07-05T12:00:00.000Z",
      reviewNotes: [
        "Adversarial fixture approving the exact transition against an older Git base.",
  ...
  const olderPlan = buildRegulatoryRegistryImplementationPlan(
    ...
    draft,
    {
      baseCommitSha: OLDER_BASE_COMMIT,
      createdAt: "2026-07-06T12:00:00.000Z",
      preparedBy: "SubShield controlled implementation planner",
    }
  );
  const olderFiles = [...new Set(olderPlan.steps.map((step) => step.targetFile))].map(
    (targetPath) => ({
      path: targetPath,
      content: execFileSync("git", ["show", `${OLDER_BASE_COMMIT}:${targetPath}`], {
        encoding: "utf8",
      }),
    })
  );
  checkRejects(
    "an older reviewed base containing stale target objects is refused",
