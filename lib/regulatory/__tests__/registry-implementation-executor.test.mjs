...
function createMemoryAdapter(baseCommitSha, baseFiles, options = {}) {
  ...
  const adapter = {
    ...
    async createCommit(branch, message) {
      ...
      commits.set(commitSha, { baseCommitSha, message, files: snapshot });
...
try {
  ...
  const executionRequest = {
    executedAt: "2026-07-07T12:00:00.000Z",
  ...
  check(
    ...
    success.status === "success" &&
      ...
      success.receipt.files.every((file) =>
        bundle.files.some(
          (candidate) =>
            candidate.path === file.path &&
            candidate.beforeChecksum === file.beforeChecksum &&
            candidate.afterChecksum === file.afterChecksum
        )
      )
  );
  check(
    "pull-request metadata is exact and auto-merge remains disabled",
    success.status === "success" &&
  ...
  const repeatAdapter = createMemoryAdapter(BASE_COMMIT, files);
  ...
  check(
    "identical inputs reproduce deterministic commit, metadata, and receipt",
    success.status === "success" &&
  ...
  const commitMismatchAdapter = createMemoryAdapter(BASE_COMMIT, files, {
    commitContentMismatchPath: bundle.files[0].path,
  });
  const commitMismatch = await executeRegulatoryImplementationPullRequest(
    plan,
    bundle,
    commitMismatchAdapter,
    executionRequest
  );
  check(
    "a commit tree that differs from the bundle is refused",
    commitMismatch.status === "execution-failed" &&
      commitMismatch.stage === "commit-verification"
  );
  ...
  const alteredPrAdapter = createMemoryAdapter(BASE_COMMIT, files, {
    pullRequestTitleMismatch: true,
  });
  const alteredPr = await executeRegulatoryImplementationPullRequest(
    plan,
    bundle,
    alteredPrAdapter,
    executionRequest
  ...
  check(
    "altered pull-request metadata is refused",
    alteredPr.status === "pull-request-failed"
  );

  const autoMergeAdapter = createMemoryAdapter(BASE_COMMIT, files, {
    autoMergeMismatch: true,
  });
  const autoMerge = await executeRegulatoryImplementationPullRequest(
    plan,
    bundle,
    autoMergeAdapter,
    executionRequest
  ...
  check(
    "an adapter that enables auto-merge is refused",
    autoMerge.status === "pull-request-failed"
  );
} finally {
