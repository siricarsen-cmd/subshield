...
try {
  ...
  for (const [overrides, message] of [
    ...
    [{ mergeCommitEnabled: true }, "merge-commit repository policy is refused"],
    [{ rebaseEnabled: true }, "rebase repository policy is refused"],
  ]) {
    ...
    await rejects(
      () =>
        overrides.mergeCommitEnabled || overrides.rebaseEnabled
          ? associationAdapter.authenticate()
          : associationAdapter.inspectExactPullRequest(57),
      /repository identity was invalid|workflow pull request evidence was foreign/,
      message
    );
  }
} finally {
