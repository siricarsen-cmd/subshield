...
if (endpoint === "repos/siricarsen-cmd/subshield" && !api.includes("--method")) output({
  ...
  allow_squash_merge: state.squashDisabled !== true,
  allow_merge_commit: state.mergeCommitEnabled === true,
  allow_rebase_merge: state.rebaseEnabled === true,
...
if (endpoint === "repos/siricarsen-cmd/subshield/git/ref/heads/main") {
  output({ object: { sha: state.merged ? state.mergeSha : (state.remoteMainSha || state.baseSha) } });
}
if (endpoint === "repos/siricarsen-cmd/subshield/git/ref/heads/" + encodeURIComponent(state.headBranch)) {
  output({ object: { sha: state.headRefSha || state.headSha } });
}
if (endpoint === "repos/siricarsen-cmd/subshield/commits/" + state.headSha) {
  output({ parents: [{ sha: state.parentSha || state.baseSha }], commit: { tree: { sha: state.headTreeSha || "c".repeat(40) } } });
}
...
if (api.includes("--method") && endpoint === "repos/siricarsen-cmd/subshield/pulls/57/merge") {
  ...
  if (state.failMerge) {
    if (state.ambiguousAccepted) state.merged = true;
    fail("uncertain transport");
  }
  if (state.mergeRefused) {
    if (state.alreadyMerged) state.merged = true;
    output({ merged: false, message: "refused" });
  }
  state.merged = true;
  output({ merged: true, sha: state.returnedMergeSha || state.mergeSha, message: "merged" });
}
...
async function createMergeAdapter(snapshot, options = {}) {
  ...
  const runtime = await createMergeRuntime({
    ...
    mergeSha: options.mergeCommitSha ?? "f".repeat(40),
