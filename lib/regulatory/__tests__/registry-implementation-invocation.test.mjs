...
const isolatedSource = source
  .replace(
    /import\s+\{[\s\S]*?\}\s+from "\.\/registry-implementation-production-adapter";/,
    isolatedDependencyStubs
  )
  .replace(
    /import\s+\{[\s\S]*?\}\s+from "\.\/registry-implementation-plan";/,
    ""
  )
  .replace(
    /import\s+\{[\s\S]*?\}\s+from "\.\/registry-implementation-pr-bundle";/,
    ""
  )
  .replace(
    /import\s+\{[\s\S]*?\}\s+from "\.\/registry-integrity";/,
    ""
  );
assert.ok(
