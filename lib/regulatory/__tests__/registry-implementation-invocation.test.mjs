...
const monotonicNow = () => globalThis.__subshieldTestMonotonicNowNs;
const executeRegulatoryImplementationWithProductionAdapter = async () => {
  globalThis.__subshieldProductionAdapterInvocations += 1;
  return ({
...
globalThis.__subshieldProductionAdapterInvocations = 0;
...
try {
  ...
  testNow = testBaseMs - 60 * 60_000;
  globalThis.__subshieldTestMonotonicNowNs = BigInt(5 * 60_000 + 1) * BigInt(1_000_000);
  const invocationsBeforeExpiry = globalThis.__subshieldProductionAdapterInvocations;
  ...
  equal(
    futureSkewInvocation.status,
    "invocation-refused",
    "future-skew authorization expires after five minutes of process-observed time"
  );
  equal(
  ...
  const backwardAuthorization = createRegulatoryImplementationInvocationAuthorization(
    fakePlan,
    fakeBundle,
    {
      authorizedAt: testIso(), expectedGitHubLogin: "SiriCarsen-Cmd",
  ...
  globalThis.__subshieldTestMonotonicNowNs -= BigInt(1);
  const backwardInvocation = await executeRegulatoryImplementationInvocation(
  ...
  testNow = testBaseMs + 5 * 60_000;
  globalThis.__subshieldTestMonotonicNowNs += BigInt(5 * 60_000) * BigInt(1_000_000);
  const withinWindow = await executeRegulatoryImplementationInvocation(
    fakePlan,
    fakeBundle,
    liveAuthorization
  );
  equal(withinWindow.status, "invocation-failed", "non-skewed authorization remains usable at five minutes");

  testNow = testBaseMs;
  globalThis.__subshieldTestMonotonicNowNs = BigInt(30 * 60_000) * BigInt(1_000_000);
  const expiringAuthorization = createRegulatoryImplementationInvocationAuthorization(
    fakePlan,
    fakeBundle,
    {
