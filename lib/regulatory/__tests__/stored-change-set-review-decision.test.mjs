...
try {
  ...
  check(
    ...
    isReverifiedStoredRegulatoryChangeSetReviewReceipt(liveReceipt) &&
      ...
      liveHumanRecord.reviewerPrincipal === "alex rivera" &&
      liveHumanRecord.sourceReviewerPrincipal === "morgan ellis" &&
  ...
  check(
    ...
    commandResult.reviewerPrincipal === "alex rivera" &&
  ...
  check(
    "the loaded audit record binds stable principals and all evidence without source bodies",
    loadedRecord.reviewerPrincipal === "alex rivera" &&
      loadedRecord.sourceReviewerPrincipal === "morgan ellis" &&
    ...
    () =>
      Promise.resolve(
        buildStoredRegulatoryChangeSetReviewRecord(
          ...
          {
            ...approvalRequest,
            reviewedBy: "SubShield automation bot",
            reviewerPrincipal: "SubShield automation bot",
          }
        )
      ),
  ...
  await checkRejects(
    "the same source reviewer cannot bypass separation by changing the role suffix",
    () =>
      Promise.resolve(
        buildStoredRegulatoryChangeSetReviewRecord(
          loadedDraft,
          ...
          {
            ...approvalRequest,
            reviewedBy: "Morgan Ellis, independent registry reviewer",
            reviewerPrincipal: "Morgan Ellis",
          }
        )
      ),
    /reviewer principal must be independent from the source reviewer and draft preparer/i
  );
  await checkRejects(
    "a claimed principal must match the reviewer display label",
    () =>
      Promise.resolve(
        buildStoredRegulatoryChangeSetReviewRecord(
          loadedDraft,
          ...
          pair,
          draftReceipt,
          { ...approvalRequest, reviewerPrincipal: "Different Human" }
        )
      ),
    /principal does not match the reviewer display label/i
  );
  await checkRejects(
  ...
  check(
    ...
    isReverifiedStoredRegulatoryChangeSetReviewReceipt(rejectionReceipt) &&
      ...
      loadedRejection.reviewerPrincipal === "jamie patel" &&
