...
function candidate(overrides = {}) {
  ...
  if (overrides.reviewStatus === "approved") {
    ...
    next.reviewedAt = overrides.reviewedAt ?? "2026-08-01T13:00:00.000Z";
...
function intake(next, overrides = {}) {
  return prepareRegulatoryUpdateIntake({
    ...
    createdAt: overrides.createdAt ?? "2026-08-01T14:00:00.000Z",
  });
}
...
check(
  ...
  metadataResult.status === "proposal-prepared" &&
    ...
    metadataResult.proposal?.trustSource === "benchmark-approved-evidence" &&
...
check(
  "a substantive in-passage source change is re-extracted into a valid draft transition",
...
check(
  "caller-supplied approval fields do not make an unretained candidate change-set-ready",
  approvedChangedResult.status === "proposal-prepared" &&
    ...
    approvedChangedResult.proposal.candidateRetainedAsApprovedEvidence === false &&
...
for (const historicalStatus of ["proposed", "superseded", "archived"]) {
  const historicalCandidate = candidate({
    idSuffix: historicalStatus,
    historicalStatus,
  });
  const historicalResult = intake(historicalCandidate);
  check(
    `${historicalStatus} source text cannot replace the current controlled source`,
    historicalResult.status === "refused" &&
      historicalResult.refusalReasons.some((reason) =>
        new RegExp(`historical status ${historicalStatus} cannot update`, "i").test(reason)
      )
  );
}
...
check(
  ...
  rejectedResult.status === "refused" &&
    rejectedResult.refusalReasons.some((reason) =>
      /unsupported regulatory update candidate review status.*rejected/i.test(reason)
    )
);

const pendingWithReview = candidate({ idSuffix: "pending-with-review" });
pendingWithReview.reviewedBy = "Premature reviewer";
pendingWithReview.reviewedAt = "2026-08-01T13:00:00.000Z";
pendingWithReview.reviewNotes = ["Pending candidate must not claim final review."];
const pendingWithReviewResult = intake(pendingWithReview);
check(
  "pending candidates cannot contain final reviewer provenance",
  pendingWithReviewResult.status === "refused" &&
    pendingWithReviewResult.refusalReasons.some((reason) =>
      /pending update candidates must not contain final reviewer provenance/i.test(reason)
    )
);
...
const createdBeforeRetrieval = intake(candidate({ idSuffix: "future-created" }), {
  createdAt: "2026-08-01T11:59:59.000Z",
});
check(
  "an intake record cannot predate its official-source retrieval",
  createdBeforeRetrieval.status === "refused" &&
    createdBeforeRetrieval.refusalReasons.some((reason) =>
      /cannot be created before the candidate was retrieved/i.test(reason)
    )
);

const approvedBeforeRetrieval = candidate({
  idSuffix: "bad-review-time",
  reviewStatus: "approved",
  reviewedAt: "2026-08-01T11:00:00.000Z",
});
const approvedBeforeRetrievalResult = intake(approvedBeforeRetrieval);
check(
  "approved candidate review provenance must follow retrieval",
  approvedBeforeRetrievalResult.status === "refused" &&
    approvedBeforeRetrievalResult.refusalReasons.some((reason) =>
      /review timestamp must follow retrieval/i.test(reason)
    )
);

const alteredBaseline = structuredClone(baseline);
alteredBaseline.provenanceNotes = [...alteredBaseline.provenanceNotes, "Caller-added note."];
const alteredBaselineResult = intake(candidate({ idSuffix: "altered-baseline" }), {
  baseline: alteredBaseline,
});
check(
  "an approved-looking baseline must exactly match retained approved evidence",
  alteredBaselineResult.status === "refused" &&
    alteredBaselineResult.refusalReasons.some((reason) =>
