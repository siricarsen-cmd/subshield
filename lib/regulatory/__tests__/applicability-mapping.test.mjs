...
check(
  "clause snapshot does not inherit unverified FAC metadata from the FAR index entry",
  pendingSnapshot.versionIdentifier === undefined && pendingSnapshot.effectiveDate === undefined
);

const farIndexHtml = `<!doctype html><html><body><main>
<h1>Federal Acquisition Regulation</h1>
<p>Federal Acquisition Circular 2026-01</p>
<p>Effective March 13, 2026</p>
</main></body></html>`;
const versionedPendingSnapshot = await fetchApprovedRegulatorySource("far-current", {
  fetchImpl: async () =>
    new Response(farIndexHtml, {
      status: 200,
      headers: { "content-type": "text/html" },
    }),
  now: new Date("2026-07-25T18:10:00.000Z"),
});
await checkRejects(
  "approval cannot omit retained version metadata",
  () =>
    Promise.resolve(
      reviewRegulatorySnapshot(versionedPendingSnapshot, {
        decision: "approved",
        reviewedBy: "SubShield regulatory reviewer",
        reviewedAt: "2026-07-25T19:10:00.000Z",
        reviewNotes: ["Verified the FAR index identity and current circular metadata."],
        requiredTextAnchors: [
...
const versionedApprovedSnapshot = reviewRegulatorySnapshot(versionedPendingSnapshot, {
  decision: "approved",
  reviewedBy: "SubShield regulatory reviewer",
  reviewedAt: "2026-07-25T19:10:00.000Z",
  reviewNotes: ["Verified the FAR index identity, circular number, and effective date."],
  requiredTextAnchors: [
    "Federal Acquisition Regulation",
    "Federal Acquisition Circular 2026-01",
  ],
...
check(
  "approval succeeds when all retained version metadata is explicitly verified",
  versionedApprovedSnapshot.reviewStatus === "approved"
);

await checkRejects(
  "review rejects version metadata that is not retained on the clause snapshot",
  () =>
    Promise.resolve(
      reviewRegulatorySnapshot(pendingSnapshot, {
        ...
        reviewedBy: "SubShield regulatory reviewer",
        reviewedAt: "2026-07-25T19:00:00.000Z",
        reviewNotes: ["Reviewed source text and governing metadata."],
        requiredTextAnchors: [
          "52.222-6 Construction Wage Rate Requirements",
          "pay all laborers and mechanics not less than the applicable wage determination",
        ],
  ...
  /version identifier does not match/i
);
await checkRejects(
  ...
  () =>
    Promise.resolve(
      reviewRegulatorySnapshot(pendingSnapshot, {
        ...
        requiredTextAnchors: [
          "52.222-6 Construction Wage Rate Requirements",
          "This required clause language is absent",
        ],
      })
    ),
...
const outputRoot = await mkdtemp(path.join(tmpdir(), "subshield-regulatory-review-"));
try {
  const storedPending = await storeRegulatorySnapshot(outputRoot, pendingSnapshot);
  check("pending snapshot is stored before review", storedPending.status === "stored");

  const persistedApproval = await persistRegulatorySnapshotReview(
    outputRoot,
    approvedSnapshot
  );
  check("approved review transition is persisted", persistedApproval.status === "approved");
  check(
    "persisted approval sets the manifest latest-approved pointer",
    persistedApproval.manifest.latestApprovedSnapshotId === approvedSnapshot.snapshotId
  );
  check(
    "persisted approval retains reviewer identity and notes",
    persistedApproval.snapshot.reviewedBy === "SubShield regulatory reviewer" &&
      persistedApproval.snapshot.reviewNotes?.length === 2
  );

  const loadedApproved = await loadLatestObservedRegulatorySnapshot(
    outputRoot,
    pendingSnapshot.sourceId
  );
  check(
    "normal snapshot loading returns the persisted approved state",
    loadedApproved?.reviewStatus === "approved" &&
      loadedApproved.reviewedAt === "2026-07-25T19:00:00.000Z"
  );

  const immutableFile = JSON.parse(await readFile(storedPending.snapshotPath, "utf8"));
  check(
    "persisting review leaves the immutable retrieved snapshot file pending and unchanged",
    immutableFile.reviewStatus === "pending" &&
      immutableFile.checksum === pendingSnapshot.checksum &&
      immutableFile.text === pendingSnapshot.text
  );

  await checkRejects(
    "reviewed snapshots cannot bypass the dedicated persisted-review transition",
    () => storeRegulatorySnapshot(outputRoot, approvedSnapshot),
    /must be stored pending/i
  );

  const changedHtml = `<!doctype html><html><body><main>
<h1>52.222-6 Construction Wage Rate Requirements</h1>
<p>The Contractor shall pay laborers and mechanics the updated applicable wage determination.</p>
<p>This fixture represents a separate pending source version.</p>
</main></body></html>`;
  const changedPending = await fetchApprovedRegulatorySource("far-52-222-6", {
    fetchImpl: async () =>
  ...
  await storeRegulatorySnapshot(outputRoot, changedPending);
  const rejectedSnapshot = reviewRegulatorySnapshot(changedPending, {
    decision: "rejected",
    reviewedBy: "SubShield regulatory reviewer",
    reviewedAt: "2026-07-26T19:00:00.000Z",
    reviewNotes: ["Rejected because the source fixture does not preserve the expected operative wording."],
    requiredTextAnchors: ["This fixture represents a separate pending source version."],
  });
  const persistedRejection = await persistRegulatorySnapshotReview(
    outputRoot,
    rejectedSnapshot
  );
  check("rejected review transition is persisted", persistedRejection.status === "rejected");
  check(
    "rejecting a later snapshot preserves the prior latest-approved pointer",
    persistedRejection.manifest.latestApprovedSnapshotId === approvedSnapshot.snapshotId
  );
  const loadedRejected = await loadLatestObservedRegulatorySnapshot(
    outputRoot,
    changedPending.sourceId
  );
  check(
    "normal snapshot loading returns the persisted rejected state",
    loadedRejected?.reviewStatus === "rejected" &&
      loadedRejected.reviewNotes?.some((note) => /Rejected because/i.test(note))
  );

  const finalManifest = await loadRegulatorySnapshotManifest(
    outputRoot,
    pendingSnapshot.sourceId
  );
  check(
    "manifest retains both final review decisions with reviewer provenance",
    finalManifest.snapshots.filter((entry) => entry.reviewStatus !== "pending").length === 2 &&
      finalManifest.snapshots.every(
        (entry) =>
          entry.reviewStatus === "pending" ||
          Boolean(entry.reviewedBy && entry.reviewedAt && entry.reviewNotes?.length)
      )
  );
} finally {
