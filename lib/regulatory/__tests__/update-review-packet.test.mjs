...
try {
  ...
  const renamedRelativePath = `${SOURCE_ID}/2026-08-02-${"0".repeat(16)}.json`;
  const renamedPath = path.join(outputRoot, ...renamedRelativePath.split("/"));
  await writeFile(renamedPath, storedJson, "utf8");
  await checkRejects(
    "packet loading rejects a valid packet copied to a misleading valid-looking path",
    () => loadRegulatoryUpdateReviewPacket(outputRoot, renamedRelativePath),
    /does not match its checksum-derived canonical path/i
  );
...
const malformedProposalTimestamp = structuredClone(packet);
malformedProposalTimestamp.proposal.createdAt = "not-an-iso-timestamp";
check(
  "packet validation explicitly rejects malformed proposal timestamps",
  validateRegulatoryUpdateReviewPacket(malformedProposalTimestamp).some((error) =>
    /proposal createdAt must be ISO/i.test(error)
  )
);
