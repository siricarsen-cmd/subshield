...
try {
  ...
  await checkRejects(
    ...
    /predates a retained approved snapshot|no earlier approved stored regulatory baseline/i
  );
} finally {
  await rm(staleRoot, { recursive: true, force: true });
}

const rollbackRoot = await mkdtemp(path.join(tmpdir(), "subshield-rollback-stored-pair-"));
try {
  const june = await snapshot(
    baselineSentence,
    "2026-06-01T12:00:00.000Z",
    '"rollback-june"'
  );
  await storeRegulatorySnapshot(rollbackRoot, june);
  await persistRegulatorySnapshotReview(
    rollbackRoot,
    approve(june, "2026-06-01T13:00:00.000Z", baselineSentence)
  );

  const august = await snapshot(
    laterSentence,
    "2026-08-01T12:00:00.000Z",
    '"rollback-august"'
  );
  await storeRegulatorySnapshot(rollbackRoot, august);
  await persistRegulatorySnapshotReview(
    rollbackRoot,
    approve(august, "2026-08-01T13:00:00.000Z", laterSentence)
  );

  const julyStoredLast = await snapshot(
    candidateSentence,
    "2026-07-01T12:00:00.000Z",
    '"rollback-july-stored-last"'
  );
  await storeRegulatorySnapshot(rollbackRoot, julyStoredLast);
  await persistRegulatorySnapshotReview(
    rollbackRoot,
    approve(julyStoredLast, "2026-09-01T13:00:00.000Z", candidateSentence)
  );
  await checkRejects(
    "a latest-observed candidate older than another retained approval cannot draft a rollback",
    () => loadVerifiedStoredRegulatoryUpdatePair(rollbackRoot, SOURCE_ID),
    /predates a retained approved snapshot.*rollback proposal/i
  );
} finally {
...
const malformedStatusRoot = await mkdtemp(
  path.join(tmpdir(), "subshield-malformed-status-stored-pair-")
);
try {
  const baseline = await snapshot(
    baselineSentence,
  ...
  await storeRegulatorySnapshot(malformedStatusRoot, baseline);
  await persistRegulatorySnapshotReview(
    malformedStatusRoot,
    approve(baseline, "2026-07-01T13:00:00.000Z", baselineSentence)
  );
  ...
  const stored = await storeRegulatorySnapshot(malformedStatusRoot, candidate);
  const manifest = JSON.parse(await readFile(stored.manifestPath, "utf8"));
  const candidateEntry = manifest.snapshots.find(
    (entry) => entry.snapshotId === candidate.snapshotId
  );
  candidateEntry.reviewStatus = "rejected ";
  candidateEntry.reviewedBy = "Independent regulatory reviewer";
  candidateEntry.reviewedAt = "2026-08-01T13:00:00.000Z";
  candidateEntry.reviewNotes = ["Malformed runtime status must never issue a verified pair."];
  await writeFile(stored.manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  await checkRejects(
    "unrecognized stored candidate review statuses are refused before opaque verification",
    () => loadVerifiedStoredRegulatoryUpdatePair(malformedStatusRoot, SOURCE_ID),
    /unsupported review status/i
  );
} finally {
  await rm(malformedStatusRoot, { recursive: true, force: true });
}
