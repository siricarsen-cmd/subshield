...
try {
  ...
  await checkRejects(
    "approval cannot invent version metadata absent from the retained snapshot",
    () =>
      recordRegulatorySnapshotReviewDecision(
        approvalRequest(metadataRoot, snapshot, {
          verifiedVersionIdentifier: "Fabricated version",
        })
      ),
    /version identifier does not match the retained snapshot/i
  );
  await checkRejects(
    ...
    /effective date does not match the retained snapshot/i
  );
} finally {
...
try {
  ...
  const newerText = `${retained.text}\nControlled later source-version marker.`;
